...
from llm import extract_keywords_with_llm
...
def fetch_nature_author(url) :
    ...
    # ---------- 2. 锁定 Author information ----------
    author_section = soup.select_one(
        'section[data-title="Author information"]'
    )
    if not author_section:
        return None, None

    # ---------- 3. 找到 affiliation 列表 ----------
    affiliations = author_section.select(
        "ol.c-article-author-affiliation__list > li"
    )
    if not affiliations:
        return None, None
    ...
    affiliation = first_aff.select_one(
        "p.c-article-author-affiliation__address"
    ).get_text(strip=True)

    authors_text = first_aff.select_one(
        "p.c-article-author-affiliation__authors-list"
    ).get_text(" ", strip=True)
    ...
    return first_author, affiliation,country
...
def send_email(file_path: str, label: str):
    ...
    if not all([smtp_server, user, password, to_email]):
        print("Email env not set, skip sending.")
        return

    msg = MIMEMultipart()
    msg["From"] = user
    msg["To"] = to_email
    msg["Subject"] = f"Nature 月度论文导读 ({label})"

    body = f"附件是 {label} 的 Nature 月度论文导读。"
...
def main():
    ...
    #papers = papers[:3]
    for p in papers:
        abstract = fetch_nature_abstract(p['url'],timeout=30)
        ...
        # 获取第一作者 & 第一机构&国家
        first_author, first_affiliation,country = fetch_nature_author(p['url'])

        p["first_author"] = first_author
        p["first_affiliation"] = first_affiliation
        p["country"]=country
        ...
        keywords = extract_keywords_with_llm(p['title'], abstract)
        p["keywords"] = keywords
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(md_text)
    send_email(output_path, label)
    print(f"Markdown exported to {output_path}")

    output_path_data = os.path.join("../nature_site/data", f"nature_{label}.md")
    with open(output_path_data, "w", encoding="utf-8") as f:
        f.write(md_text)
    print(f"Markdown also exported to {output_path_data}")
